...
// byte.
import { isChromeMessages } from "./messages";
import { addLeaf, checkPath, deleteKey, deleteLeaf, editLeaf } from "./splice";
...
// A BOM is kept in the text, which the splice edits around; only
...
export function entriesToMessages(
  ...
  if (isChromeMessages(parseTree(template)))
    return chromeMessages(template, translations, existing);
...
function chromeMessages(
  ...
  for (const [id, next] of Object.entries(translations)) {
    ...
    const model = source[id];
    const fields: [string[], string][] =
      typeof model === "object" ? leaves(model) : [[["message"], next]];
...
export function applyMessagesOps(text: string, ops: SourceOp[]): string {
  ...
  if (isChromeMessages(tree)) return chromeOps(text, ops);
  const nested = isNested(tree);
  const { indent } = styleOf(text);
  let out = text;
  for (const op of ops) {
