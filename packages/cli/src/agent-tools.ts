...
export function tools(api: Api): Tool[] {
  ...
  return [
    ...
    },
    {
      ...
      op: "string",
      description:
        "One string as the editor shows it: source text, its library (icu; i18next with {{name}} interpolation; vue with pipe plurals and {'…'} literals; printf with %s and %[2]s verbs named by their position, each slot carrying the verb as written; or chrome with $NAME$ placeholders named without regard to case; a translation writes placeholders the same way), placeholders, selects, plurals and the rich-text tags a translation must keep (unless richText is html: the type is read as HTML and a translation may write its own tags), each slot with what the repository declares for it (description and role) and its first example value per language, examples with their values per language, every language's text and state, the string's own note from the repository and the type's note on voice and register, the glossary terms that occur in the source with their target renderings, the entities it refers to (characters, rooms and the like) with their names and attributes, any pending proposal, and its siblings (the ten nearest strings of the same type under the same key prefix, or in the same i18next plural family, with their translations) so a set reads as one.",
      inputSchema: {
        type: "object",
        properties: { key },
        required: ["key"],
