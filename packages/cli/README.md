...
A package manager with a release-age policy (pnpm's `minimumReleaseAge`, say) refuses a version published inside its window; install the previous release, or wait it out. `init` writes `corpus.config.ts` and adds `.corpus/` to `.gitignore`, creating the file when there is none, as `workbench` does too. `workbench` starts an instance from the companion package, creates the project the config declares and writes the token to `.corpus/token`. Open the URL, join with the printed secret, translate and verify; `pull` writes the verified rows back, format-preserving, and a second pull changes nothing. For a team's instance, `CORPUS_INVITE_SECRET=<secret> npx corpus project create` prints the project's token once, for `CORPUS_TOKEN` or `.corpus/token`.
