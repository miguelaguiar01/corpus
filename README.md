...
A package manager with a release-age policy (pnpm's `minimumReleaseAge`, say) refuses a version published inside its window; install the previous release, or wait it out. `init` writes `corpus.config.ts`, the whole configuration for a repository whose strings are a plain message catalog. `workbench` starts an instance at http://localhost:3000, creates the project the config declares, and prints the invite secret it generated; the project's token is written to `.corpus/token`, so in another shell:
