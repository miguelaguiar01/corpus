...
Open the URL, join with the secret, a display name and a password, and you are the maintainer: translate, verify, then `npx corpus pull` writes the verified translations back into the repository's files. The database, the secret and the token live under `.corpus/`, which `init` and `workbench` both add to `.gitignore`, creating the file when there is none, so the token is ignored on the team path too, where `project create` prints it and the workbench never runs; delete the directory to start over. Updating is `npm update` of the two packages, which always share a version. A package manager with a release-age policy refuses a version published inside its window, and the three write it differently: pnpm's `minimumReleaseAge` in `pnpm-workspace.yaml`, yarn's `npmMinimalAgeGate` in `.yarnrc.yml`, npm's `min-release-age` in `.npmrc`. Each has an allow list for packages a team has decided to trust on release day, `minimumReleaseAgeExclude`, `npmPreapprovedPackages` and `min-release-age-exclude[]` respectively; pnpm adds the entry itself when you install. Otherwise install the previous release, or wait the window out.
