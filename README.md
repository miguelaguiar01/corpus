...
Corpus translates its own interface with itself. That is the standing demo in these screenshots and a test that runs on every build. The screenshots stage a frozen copy of the interface catalogue, so they move only when the interface does; `bin/screenshots --refresh-fixture` takes a new copy when the catalogue's growth is worth showing, and `bin/screenshots` regenerates the images.
