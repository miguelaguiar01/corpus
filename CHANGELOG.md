...
- A `messages` source reads Flutter's `.arb`: JSON under another name, whose top-level keys starting with `@` (`@wallpaper`, `@@locale`) are metadata for their sibling rather than strings. Ente's 2,142-key catalogue in 57 languages failed on 0.17.0 with `Unknown file extension ".arb"` and needed an exec converter; it now reads as a `messages` source, `pull` writes the `.arb` back in place with the `@` entries untouched, and `init` detects the library through it. A description under `@key` is not carried yet, since an entry has no note of its own (#567).
