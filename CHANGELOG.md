...
- A push of any size the cap allows gets through: the cap is 128 MiB on the inflated body, from 32, and the instance's proxy takes a request body up to the same size, where Next's default cut one past 10 MB, so Bitwarden's 8,617 strings with their translations in 67 languages, 48 MiB of JSON and 11 MiB gzipped, answered `HTTP 413` on 0.18.0 at the old cap and would have been refused as invalid gzip past it. The cap only keeps a token holder from parking a gigabyte in memory, and it still does.
