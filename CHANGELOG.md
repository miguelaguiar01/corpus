...
- Proposals on the agent path (§3, §10): `GET /api/proposals` lists the pending ones with their authors, the agent actor's own marked; `DELETE /api/proposals/<id>` withdraws one of the actor's own. `list_proposals` and `withdraw_proposal` tools, `proposals` and `withdraw` subcommands and stdin ops.
