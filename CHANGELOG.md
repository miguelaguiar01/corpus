...
- The dashboard's table, shown from eight languages, keeps the per-type breakdown: a project with more than one string type opens the per-type bars behind the row's language, closed by default, so a many-language project sees one bar per language and the breakdown on demand (§9.1).
