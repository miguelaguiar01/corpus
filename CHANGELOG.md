...
- Chrome i18n's `messages.json`, the catalogue of every browser extension, is a `messages` format: the adapter reads an object of `{ message, description?, placeholders? }` entries by its shape, `message` as the text, `description` as the note and each placeholder's `example` as its example value, and a pull writes the `message` back in place, keeping the rest of the entry and a byte-order mark, a key new to a language copying the source's `description` and `placeholders`. Under `library: "chrome"` `$NAME$` is a placeholder, matched without regard to case as Chrome matches it, and `$$` a dollar; `init` names the library from the shape. Bitwarden's four apps, 8,617 strings in 67 languages, push through two `{ns}` patterns with no converter and pull back byte-identical; in the 0.18.0 trial they took a converter that rewrote `$NAME$` to `{NAME}` and back.
