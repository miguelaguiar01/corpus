...
- After the i18next syntax: the string page's key heading keeps a key's line breaks; i18next's unescaped form `{{- name}}` is read as the placeholder `name`; the proposal refusal and the editor's message say the text must parse as a source in the string's syntax instead of naming ICU; §5's example entry lists `file` and `syntax`.
