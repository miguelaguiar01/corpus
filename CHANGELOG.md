...
- A `messages` or `table` source may name several files: `path` takes an array of patterns, one source per pattern sharing the type and library, and a pattern may carry `{ns}` for the layout with one file per namespace that i18next uses by default (`locales/{lang}/{ns}.json`), one source per namespace found, its ids prefixed `ns:` with i18next's own separator. A config used to need a source per namespace and a new entry whenever one was added. `pull` writes each string back to the file its id names and never lets one file's strings leak into another of the same type; `init` reads the languages and the library through a `{ns}` pattern and writes it as given.
