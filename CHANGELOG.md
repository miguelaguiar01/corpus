...
- The wiki's version strings cannot lag a release: the `/api/health` answer on *A team instance* is recorded by `bin/wiki-record-live` (it said `v0.16.0` by hand, two releases behind), and the wiki test asserts that every version in the recorded sessions and the health answer equals `packages/cli/package.json`'s, so a release branch's gate fails until the recordings are re-taken. The MCP and agent recordings are re-taken here, and now carry `keyIsText`.
