...
- The dashboard's table leads with the language that has the most left to translate, the source language pinned first, instead of ordering by code. On Jellyfin's 108 languages the one that needs a maintainer, Luxembourgish with 63 of the project's validation findings, sat sixty rows down among forty complete ones.
