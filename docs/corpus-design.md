...
- **M6 — Ship as a package.** The CLI and the contract it imports are published to npm, built to plain JavaScript with declarations, with `corpus init` scaffolding a config, a release workflow, and an install smoke in CI that installs the published tarballs into a fresh repository and round-trips against the container. *Done when: `npm install` of the CLI in a fresh repository, `corpus init`, `corpus push` and `corpus pull` reproduce that repository byte for byte, verified by a CI job, and the README's quick start is that path.*
