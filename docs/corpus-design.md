...
Two more things inform a translator and belong to the repository (§4). A **type note** (`typeNotes` in the config, keyed by string type) is one sentence on the voice and register of everything of that type, "the judge's remarks are dry noir", shown under the source of every string of the type; a note lives on the type only, a single string's note is a `text` field. A **glossary** (`glossary: { path }` in the config, one JSON file per target language, an array of `{ term, target, note? }`) pins the words that must stay the same across a thousand strings; the string page and the agent's view show the entries whose `term` occurs in the source, matched case- and accent-insensitively on whole words, with the target rendering and the note. Nothing enforces the glossary yet (§13).
