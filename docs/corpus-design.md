...
3. **Editor** — source left (branch view for selects, placeholder chips, metadata chips, entity cards in a side rail, examples), target right (chip-insertable placeholders — no hand-typed braces), live preview from the draft using example values (the selected language's when the example carries them, §7), save (→ `translated`) and verify (→ `verified`, maintainer only), visible attributed history, next/previous at thumb height to flow through a queue. Validation feedback is inline and specific ("missing {witness}"). The per-language state chips at the top are the **language switcher**: one per language, the selected one marked; the queue the person came from is kept when the switched-to row is in it, and dropped otherwise, so next and previous never point at a row the queue does not contain; the source language's chip is the proofreading view. Placeholder chips keep the token as their text and show the resolved value on hover (§7). Under the source, anyone may **propose a change** to the source text (the same chips and validation as the target pane; the text must parse as a source) or **propose removal**; a pending proposal shows its text and author, can be withdrawn, and stays visible until a push lands or supersedes it. A string whose source pull cannot write, an `exec` source or a `.ts` catalogue (§4), shows why instead of the buttons. Under the source pane, **the other languages**, every language except the source and the selected target, are readable: current text or a quiet "No translation yet", the state, stale marked; read only, so save and verify act on one target at a time.
