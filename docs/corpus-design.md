...
- **M15 — What the third MCP run found.** Glossary entries list their forms; an agent sees its pending proposals, withdraws its own, and is told what applies one. *Done when: a source that writes a listed form finds its glossary entry on the string page and in `get_string`; `corpus pull` names what applies a proposal it wrote; the token lists the pending proposals with the agent's own marked and withdraws one of its own but not a person's; released as 0.11.0.*
