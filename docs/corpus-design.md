...
- The project token also **reads and drafts**, so an agent can work without a browser: `GET /api/queues` (the three queues, per language), `GET /api/strings/<key>` (what the editor shows: source, examples, placeholders and selects, every language's text and state, the pending proposal), `PUT /api/strings/<key>/translations/<lang>` (a draft), `POST /api/strings/<key>/proposals` (an edit or a removal) and `POST /api/proposals` (a new string). Three rules hold for every write through the token. **An agent never overwrites a person's work:** a draft is accepted only on an `untranslated` or `stale` row, or on one whose last edit came through the token; a row a person edited answers `409 human-edited`, and the agent's recourse is a proposal, never a write. **Every draft is attributed:** writes through the token are recorded as the project's **agent actor**, a user the project carries from creation, named after the token, which the history and the chips show as such (§11). **Nothing but a signed-in maintainer verifies:** the token cannot verify, change settings or touch users, and a draft through it lands in the unverified queue like any other. The routes are rate-limited like push, with the same body caps, and log neither the token nor the text.
...
- **Agents:** the smoke drives the API as an agent (a draft on an `untranslated` row, a refusal on a row a person edited, a proposal) and then verifies the draft as a maintainer, seeing the attribution; a CLI test drives `corpus mcp` over stdio with the protocol's own client against the in-process server and exercises every tool.
