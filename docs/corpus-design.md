...
A string entry may carry `file`, the repository path it was read from (the source-language path of its source), which is what lets a proposal be written back to the right file; entries from `exec` sources carry none. The pull payload (§8) may carry `sourceChanges`, the pending proposals: `{ kind: "edit" | "add" | "delete", id, type, file, text? }`.
...
Push also **reconciles proposals** (§11): a pending edit whose text now equals the pushed source, a pending add whose id now exists, and a pending delete whose id is now absent become `applied`; a pending edit whose string was pushed with some other text becomes `superseded`. Proposals never change a translation or a state; the push that lands them does, as any push (stale on a changed source, archive on a missing id).
...
**Proposals** are beside the state machine, not in it. A proposal is an `edit` (a string, new text), an `add` (a key, a type, a file, text) or a `delete` (a string), by anyone, `pending` until a push makes it `applied` or `superseded` (§8) or its author withdraws it; one pending proposal per string or key, a newer one replacing the older. Proposals are read by `pull` (§8) and touch no row until the push that lands them; the pending count is on the dashboard, in `corpus status` and on the home card.
