...
A snapshot may carry `typeNotes`, one sentence per string type on voice and register, and `glossary`, per target language an array of `{ term, target, note? }` read from the file the config names with `{lang}` replaced. Both belong to the repository like source text: a push replaces them whole, pull never writes them, and a push without them leaves the project's copies in place.
...
- **M13 — What the first MCP user found.** Refusals name the next move and the writable sources; queue items carry their type; siblings, a type note and the glossary entries in the source inform a translator, person or agent, under the source and in `get_string`; `corpus agent` is the seven tools as subcommands; the README says how to connect before a session starts. *Done when: on a project pushed with a glossary and a type note, the string page and `get_string` show the note, the glossary entries in the source and up to ten siblings with their target text; `list_queue` filters by type; a refusal on a person's row says to propose instead; a proposal into a source pull cannot write names the writable ones; `corpus status` lists them; `corpus agent draft` saves a draft that the agent drafts queue shows; the Moonlight Manor fixture carries a note and a glossary so the tests, the smoke and the screenshots exercise them; released as 0.9.0.*
