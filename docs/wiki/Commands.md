...
**`corpus init`** writes `corpus.config.ts` (or `corpus.config.mjs` as a plain object when `@corpus-tool/cli` is not installed in the repository), adds `.corpus/` to `.gitignore` and prints what to do next. It detects the library from the catalogue it is pointed at by counting the placeholder shapes its strings use, and says when the placeholders are printf verbs Corpus does not check, or i18next keys with single-brace interpolation, which stay `icu` so the placeholders are checked. It writes `check.include` from the directories that hold components, of `src`, `app`, `lib`, `components` and `shared`, and says which; when only `src` does, or none, it writes no `check` key and `corpus check` reads `src`. Declare `check.include` yourself when your components live somewhere else.
