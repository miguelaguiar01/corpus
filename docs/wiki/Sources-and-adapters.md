...
A `.arb` target that does not exist yet is written on the first pull with `"@@locale"` first, set to its language, then the strings; the `@key` metadata stays in the source ARB, where `gen-l10n` reads it.
